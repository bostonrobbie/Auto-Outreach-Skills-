-- BDR Analytics DB Schema (starter kit — no data, schema only)
-- USAGE: python3 -c "import sqlite3; c=sqlite3.connect('bdr.db'); c.executescript(open('analytics/db_init.sql').read()); c.commit(); c.close()"
-- NOTE: build the DB on a LOCAL path, not a cloud-synced folder (OneDrive/Drive break SQLite file locking).
-- Load rows from MASTER_SENT_LIST.csv, memory/warm-leads.md, and memory/call-log.md.
...
CREATE INDEX IF NOT EXISTS idx_sends_sent ON outreach_sends(send_date);
