...
- **Composition/whitespace:** an email must be four short paragraphs with actual blank lines between them (never one run-on block), sign-off on its own lines. Audit the literal stored text — content that is "logically 4 parts" but stored as a single block FAILS, because it will paste as a wall of text. A CR must be a single short block. This check is on formatting, not meaning.
- Email: 4 paragraphs; {{MIN}}-{{MAX}} words; opener is a question on a verified fact; one named proof; engagement CTA (not a meeting ask); correct sign-off; no em dashes; ~2 question marks.
...
- Recipient matches the ICP (right title/level, not a wrong-persona trap); account is in the approved universe. Off-persona or off-scope → FAIL (or FLAG for the rep's call if borderline).
...
- Only a batch where every remaining draft is PASS proceeds to the pre-send flight check.
