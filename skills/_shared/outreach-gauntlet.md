...
> This is the engine underneath every channel skill (email / linkedin / calls). No message is "ready" until every gate below is checked. The agent runs the FULL gauntlet on every message, every channel, EVERY time — automatically, to standard, so drafts pass on the first review. Never a low-effort pass. This is configured from `config.json` and your `voice-profile.md`.
...
- **Scope:** state the approved account universe (TAM / Factor / intent lists) + ownership rule. Never source off-list or not-yours.
- **ICP:** state what you WANT (titles/seniority/verticals) and what you DON'T (Lead/Senior-IC/Staff excludes + wrong-persona traps).
- **Quota:** determine today's number — from the rep's explicit ask, else `config.quota` daily targets, minus what's already sent today.
...
1. **Scope:** the account is on your target list (`data/target-accounts.csv`) or an authorized exception source. No off-list accounts.
...
3. **Route → sequence/sender:** the account's tag maps to the correct sequence and sending address per `config.json` `tech_stack`.
...
8. **Persona lock:** the title matches your ICP in `config.json` (seniority floor + included titles), and is NOT an excluded pattern. Read the *live* title, not just the enrichment tool's cached one.
9. **Live profile read PER CONTACT:** open the person's live LinkedIn (or primary) profile — headline, About, experience, connection degree, recent activity. This catches: left-the-company (stale enrichment), wrong-persona (e.g. clinical/hardware/compliance QA when you sell software QA), title inflation, and profile/slug mismatches. Enrichment data is necessary but NOT sufficient. **Never skip this, even in a large batch.**
...
12. **Draft to the channel formula** (email = the 4-paragraph standard: diagnostic question → context → named proof → engagement CTA). Vary the opener SHAPE, the context construction, and the CTA across the batch so no two drafts share a frame.
...
14. **Score ≥ 9/12 on the MQS rubric** (`draft-qc-rubric.md`), zero hard-rule violations.
...
## F. Send (only after explicit approval + the pre-send flight check)
Run the **pre-send flight check** and, after each send, the **post-send maintenance check** (`preflight-postflight.md`). Pre-flight verifies approval + audit-pass + re-dedup + DNC + current-employer + verified-email + correct-sender + character-exact readback before the send fires. Post-flight verifies it landed as intended, logs MASTER, updates trackers, and sweeps for bounces/incidents.

### Legacy send notes
20. Create/verify the contact (verified email only). Enroll in the correct sequence/sender if the flow requires it.
21. Send via your tool. Pre-send readback: confirm recipient, sender address, subject, and body (including paragraph breaks) before the send fires. Verify the send succeeded.
...
- **Send nothing without the rep's explicit approval.** Build → QA → present → wait → send.
- **Verified contacts only. Your own contacts only** (never another rep's).
